...
네이버 경제지 신문보기 1면 기사 제목을 수집하고, 환경변수로 선택한 LLM provider(Codex CLI 또는 Gemini API)로 이미지 생성용 경제 키워드 후보 5개를 뽑는 독립 프로젝트입니다.
...
```powershell
python src/main.py --select-keyword-and-insert
```

이 옵션을 켜면 출력된 키워드 후보 5개를 `n8n_publish_content.keyword`에서 먼저 조회하고, 이미 존재하지 않는 후보만 남긴 뒤 경제적 이슈성/바이럴 가능성이 가장 높은 1개를 LLM으로 선별합니다. 마지막으로 `n8n_publish_content`에 `3초퀴즈`, `자녀에게설명하기` 카테고리로 2건을 insert합니다.
...
--select-keyword-and-insert
                      MySQL 중복 검사, 최종 1개 선별, insert SQL 실행까지 수행합니다.
```
